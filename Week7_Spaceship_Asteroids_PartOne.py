...
#http://www.codeskulptor.org/#user33_Hpyi27HGLGViYjO.py
#
...
class Ship:
    ...
    def shoot(self):
        ...
        a_missile.vel = [self.vel[0] + 5*forward[0], self.vel[1] + 5*forward[1]]
...
def rock_spawner():
    global a_rock
    a_rock = Sprite([randrange(0, WIDTH), randrange(0, HEIGHT)], [random.random()*1.5 + random.random()*1.5, random.random() + random.random()], 
...
a_rock = Sprite([randrange(0, WIDTH), randrange(0, HEIGHT)], [random.random()*1.5, random.random()*1.5], 0, random.random() * -.2 + .1, asteroid_image, asteroid_info)
